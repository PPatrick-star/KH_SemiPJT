...
public class UserService {
	...
	public int confirmPassword(String beforeUserPwd, String userId) {
		int result = 0;
		Connection conn = null;
		
		try {
			conn = factory.createConnection();
			result = new UserDAO().confirmPassword(conn, beforeUserPwd, userId);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	public Member selectMemberList(String userId) {
		Member member = null;
		Connection conn = null;
		
		try {
			conn = factory.createConnection();
			member = new UserDAO().selectMemberList(conn, userId);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
