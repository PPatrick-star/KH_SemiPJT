...
<body>
	<div id="right_head" style="width: 1200px; height: 72px;">
		<div id="head_title">
				<h1 style="font-weight: 900;">개인승인조회</h1>
			</div>
			
			<div id="search">
				<form action="/personal/ack/search" method="get" name="searchname" id="searchname">
		...
		<div id="right_middle">
			<table class="table">
				...
				<tbody>
				<jsp:useBean id="acknow" class="java.util.Date" />
				<fmt:formatDate value="${acknow}" pattern="yyyy-MM-dd" var="daytoday" />
					<c:forEach items="${paList }" var="personalReservation" varStatus="index">
						<tr>
							...
							<td class="btn">
								<c:if test="${personalReservation.rpState eq '승인' }">
									<c:if test="${personalReservation.rpDate<=daytoday }">
					...
					<tr>
						<td colspan="8" align="center">${personalAckPageNavi }</td>
					</tr>
				</tbody>
			</table>
		</div>
